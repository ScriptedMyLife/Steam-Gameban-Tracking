...
def add_user_embed_log(steam_id, avatar_url, author):
    """Creates a discord embed for logging user add command"""
    embed = log_command_template("/add_user")
    embed.add_field(name="Steam ID added", value=f"[{steam_id}](https://steamcommunity.com/profiles/{steam_id})")
    embed.add_field(name="Sent at", value=gen_timestamp())
    embed.set_thumbnail(avatar_url)
...
def log_command_template(cmd_name):
    ...
    embed.add_field(name="Command Name", value=f"{cmd_name}")
    return embed
